...
(async()=>{
 ...
 try{
  ...
  await frame.locator('.deck-item[aria-label="Open 물 in Objects"]').click().catch(async()=>{await frame.locator("#group-filter").getByRole("button",{name:/Objects/}).click();await frame.locator('.deck-item[aria-label="Open 물 in Objects"]').click()});
